...
export function handleTokensMintedWithSignature(
  ...
  let existingAddressMintedRecord = checkAddressMinted(event);

  if (!!existingAddressMintedRecord) {
    updateAddressMinted(existingAddressMintedRecord);
  } else {
    createAddressMinted(event);
  }
}
...
function createAddressMinted(event: TokensMintedWithSignatureEvent): void {
  ...
  entity.entries;
  entity.address = event.params.mintedTo;
  entity.numberMinted = ONE;

  entity.save();
}

function updateAddressMinted(entity: AddressMinted): void {
  entity.numberMinted = TWO;
  entity.save();
}
